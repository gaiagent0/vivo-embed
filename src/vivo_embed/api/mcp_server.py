...
async def call_tool(name: str, args: dict) -> dict:
    ...
    if name == "search_knowledge":
        emb = embedder.embed_query(args["query"])
        col = args.get("collection")
        top_k = args.get("top_k", 8)
        if col:
            raw = store.search(col, emb, n_results=top_k)
            results = [{"content": raw["documents"][0][i],
            ...
            results = [{"content": r["content"], "path": r["metadata"].get("path", ""),
                        "score": round(1.0 - r["distance"], 4), "collection": r["collection"]}
                       for r in raw]
